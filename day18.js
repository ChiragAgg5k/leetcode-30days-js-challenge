...
var jsonStringify = function (object) {
	...
	if (Array.isArray(object)) {
		...
		if (object.length !== 0) {
			// in case of empty array, no need to remove last ","
	...
	if (typeof object === "object") {
		...
		if (Object.keys(object).length !== 0) {
			// in case of empty object, no need to remove last ","
			res = res.slice(0, -1); // removing last ","
		}
...
console.log(
	jsonStringify([new Number(3), new String("false"), new Boolean(false)])
); // "[3,"false",false]"
